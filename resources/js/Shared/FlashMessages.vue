<template>
  <div>
    <div v-if="pageProps.flash.success && show" class="mb-8 flex items-center justify-between bg-green-500 rounded max-w-3xl">
      <div class="flex items-center">
        ...
        <div class="py-4 text-white text-sm font-medium">{{ pageProps.flash.success }}</div>
      </div>
    ...
    <div v-if="(pageProps.flash.error || Object.keys(pageProps.errors).length > 0) && show" class="mb-8 flex items-center justify-between bg-red-400 rounded max-w-3xl">
      <div class="flex items-center">
        ...
        <div v-if="pageProps.flash.error" class="py-4 text-white text-sm font-medium">{{ pageProps.flash.error }}</div>
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys(pageProps.errors).length === 1">There is one form error.</span>
          <span v-else>There are {{ Object.keys(pageProps.errors).length }} form errors.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
